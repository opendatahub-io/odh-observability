...
oats-bin: ## Install oats binary locally if necessary.
	@mkdir -p "$(BIN_DIR)"
	@test -s "$(OATS_BIN)" || GOBIN="$(BIN_DIR)" go install github.com/grafana/oats

.PHONY: gcx-bin
gcx-bin: ## Install gcx binary locally if necessary.
	@mkdir -p "$(BIN_DIR)"
	@test -s "$(GCX_BIN)" || GOBIN="$(BIN_DIR)" go install github.com/grafana/gcx/cmd/gcx
